...
export function buildBundleHandoffOperatorRunbook(commandManifest) {
  ...
  const stageCommandLabelsByKey = byKey(stages, (stage) => stage.commands.map((command) => command.label));
  const stageCommandStringsByKey = byKey(stages, (stage) => stage.commands.map((command) => command.command));
  const stageCommandArgsByKey = byKey(stages, (stage) => stage.commands.map((command) => command.commandArgs));
  const stageCommandRunPoliciesByKey = byKey(stages, (stage) => stage.commands.map((command) => command.runPolicy));
  const stageCommandSafetyLevelsByKey = byKey(stages, (stage) => stage.commands.map((command) => command.safety?.safetyLevel || ""));
