...
test("runHelp lists advanced options supported by command parsers", async () => {
  ...
  assert.match(output, /site <workspace\.json\|--stdin> \[--strict\] \[--json\|--tasks\|--report\|--prompts\|--prompt id \[--task id\]\] \[--out file\] \| site --sample \[--out file\] \| site --prompt-list \[--json\]/);
  assert.match(
    output,
...
test("runHelp emits a machine-readable help topic catalog", async () => {
  ...
  assert.equal(
    ...
    "design-ai site <workspace.json|--stdin> [--strict] [--json|--tasks|--report|--prompts|--prompt id [--task id]] [--out file] | site --sample [--out file] | site --prompt-list [--json]",
  );
...
test("runHelp delegates command topics to command-specific help", async () => {
  ...
  assert.match(siteOutput, /--out file\s+Write --json, --sample, --prompt-list, --tasks, --report, --prompts, or --prompt output to a file/);
