...
test("runHelp lists advanced options supported by command parsers", async () => {
  ...
  assert.match(output, /learn \[--remember text\|--list\|--export\|--forget id\|--clear\] \[--json\]/);
  assert.match(
    output,
  ...
  assert.match(
    output,
    /learn \[--remember text\|--list\|--export\|--forget id\|--clear\] \[--json\]\s+Manage local learning preferences for prompt personalization/,
  );
  assert.ok(
...
test("runHelp emits a machine-readable help topic catalog", async () => {
  ...
  assert.equal(
    ...
    "design-ai learn [--remember text|--list|--export|--forget id|--clear] [--json]",
  );
