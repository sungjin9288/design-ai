...
test("runHelp lists advanced options supported by command parsers", async () => {
  ...
  assert.match(output, /site <workspace\.json\|--stdin> \[--strict\] \[--json\|--mcp-check\|--mcp-plan\|--tasks\|--bundle\|--report\|--prompts\|--prompt id \[--task id\]\] \[--out file\] \| site <bundle-dir> --bundle-check \[--json\] \| site <bundle-dir> --bundle-compare other-bundle-dir \[--json\] \| site --sample \[--out file\] \| site --prompt-list \[--json\]/);
  assert.match(
    output,
...
test("runHelp emits a machine-readable help topic catalog", async () => {
  ...
  assert.equal(
    ...
    "design-ai learn [--init|--remember text|--feedback text|--list|--export|--query text|--explain|--backup|--redact|--verify|--import|--audit [--fix]|--stats|--forget id|--clear] [--json] [--out file]",
  );
  assert.equal(
  ...
  assert.equal(
    ...
    "design-ai site <workspace.json|--stdin> [--strict] [--json|--mcp-check|--mcp-plan|--tasks|--bundle|--report|--prompts|--prompt id [--task id]] [--out file] | site <bundle-dir> --bundle-check [--json] | site <bundle-dir> --bundle-compare other-bundle-dir [--json] | site --sample [--out file] | site --prompt-list [--json]",
  );
...
test("runHelp delegates command topics to command-specific help", async () => {
  ...
  assert.match(siteOutput, /--out file\s+Write --json, --sample, --prompt-list, --mcp-check, --mcp-plan, --tasks, --bundle, --bundle-check, --bundle-compare, --report, --prompts, or --prompt output to a file or directory/);
  ...
  assert.match(learnOutput, /--dry-run\s+Preview --init, --import, or --audit --fix without changing the profile/);
