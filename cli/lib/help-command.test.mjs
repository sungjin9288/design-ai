...
test("runHelp lists advanced options supported by command parsers", async () => {
  ...
  assert.match(output, /site <workspace\.json\|--stdin> \[--strict\] \[--json\|--mcp-check \[--probes\]\|--mcp-plan \[--probes\] \[--json\]\|--next-actions \[--json\]\|--graph\|--tasks\|--bundle\|--report\|--prompts\|--prompt id \[--task id\]\] \[--out file\] \| site <bundle-dir> --bundle-check \[--json\] \| site <bundle-dir> --bundle-compare other-bundle-dir \[--json\] \| site <bundle-dir> --bundle-handoff \[--json\] \| site <bundle-dir> --bundle-repair \[--yes\] \[--json\] \[--out file\] \| site --init --name name --live-url url \[--next-actions\] \[--out file\] \| site --init --name name --live-url url --bundle --out dir \| site --from-intake file\.md\|--stdin \[--json\|--next-actions \[--json\]\|--bundle --out dir\] \[--out file\] \| site --intake-template \[--language en\|ko\] \[--json\] \[--out file\] \| site --sample \[--out file\] \| site --prompt-list \[--json\]/);
  assert.match(
    output,
...
test("runHelp emits a machine-readable help topic catalog", async () => {
  ...
  assert.equal(
    ...
    "design-ai site <workspace.json|--stdin> [--strict] [--json|--mcp-check [--probes]|--mcp-plan [--probes] [--json]|--next-actions [--json]|--graph|--tasks|--bundle|--report|--prompts|--prompt id [--task id]] [--out file] | site <bundle-dir> --bundle-check [--json] | site <bundle-dir> --bundle-compare other-bundle-dir [--json] | site <bundle-dir> --bundle-handoff [--json] | site <bundle-dir> --bundle-repair [--yes] [--json] [--out file] | site --init --name name --live-url url [--next-actions] [--out file] | site --init --name name --live-url url --bundle --out dir | site --from-intake file.md|--stdin [--json|--next-actions [--json]|--bundle --out dir] [--out file] | site --intake-template [--language en|ko] [--json] [--out file] | site --sample [--out file] | site --prompt-list [--json]",
  );
...
test("runHelp delegates command topics to command-specific help", async () => {
  ...
  assert.match(siteOutput, /design-ai site --from-intake file\.md\|--stdin \[--json\|--next-actions \[--json\]\|--bundle --out dir\] \[--out file\] \[--strict\] \[--force\]/);
