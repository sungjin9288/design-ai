...
test("runHelp lists advanced options supported by command parsers", async () => {
  ...
  assert.match(
    output,
    /learn \[--init\|--remember text\|--feedback text\|--list\|--export\|--query text\|--explain\|--backup\|--redact\|--verify\|--diff\|--restore\|--restore-backups \[--prune\]\|--import\|--audit \[--fix\]\|--curate\|--stats\|--usage\|--signals \[--strict\]\|--propose-skills \[--min-evidence N\] \[--strict\]\|--eval-template\|--eval \[--strict\]\|--forget id\|--clear\] \[--json\|--report\|--patch\] \[--out file\]\s+Manage local learning preferences, usage reports, signal registry, skill proposals, and eval checkpoints for prompt personalization/,
  );
  assert.ok(
...
test("runHelp emits a machine-readable help topic catalog", async () => {
  ...
  assert.equal(
    ...
    "design-ai learn [--init|--remember text|--feedback text|--list|--export|--query text|--explain|--backup|--redact|--verify|--diff|--restore|--restore-backups [--prune]|--import|--audit [--fix]|--curate|--stats|--usage|--signals [--strict]|--propose-skills [--min-evidence N] [--strict]|--eval-template|--eval [--strict]|--forget id|--clear] [--json|--report|--patch] [--out file]",
  );
  assert.equal(
...
test("runHelp delegates command topics to command-specific help", async () => {
  ...
  assert.match(learnOutput, /design-ai learn --propose-skills \[--from-file signal-file-or-dir\] \[--usage-file path\] \[--min-evidence N\] \[--strict\] \[--json\|--report\|--patch\] \[--out file\] \[--force\]/);
  ...
  assert.match(learnOutput, /--strict\s+With --eval, --signals, or --propose-skills, exit non-zero when any checkpoint, signal gate, or skill proposal gate warns or fails/);
