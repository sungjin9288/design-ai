...
test("runHelp lists advanced options supported by command parsers", async () => {
  ...
  assert.match(
    output,
    /learn \[--init\|--remember text\|--feedback text\|--list\|--export\|--query text\|--explain\|--backup\|--redact\|--verify\|--import\|--audit \[--fix\]\|--curate\|--stats\|--usage\|--eval\|--forget id\|--clear\] \[--json\] \[--out file\]\s+Manage local learning preferences, usage reports, and eval checkpoints for prompt personalization/,
  );
  assert.ok(
...
test("runHelp emits a machine-readable help topic catalog", async () => {
  ...
  assert.equal(
    ...
    "design-ai learn [--init|--remember text|--feedback text|--list|--export|--query text|--explain|--backup|--redact|--verify|--import|--audit [--fix]|--curate|--stats|--usage|--eval|--forget id|--clear] [--json] [--out file]",
  );
  assert.equal(
