...
test("runHelp lists advanced options supported by command parsers", async () => {
  ...
  assert.match(output, /site <workspace\.json\|--stdin> \[--strict\] \[--json\|--tasks\|--report\|--prompts\] \[--out file\] \| site --sample \[--out file\]/);
  assert.match(
    output,
...
test("runHelp emits a machine-readable help topic catalog", async () => {
  ...
  assert.equal(
    ...
    "design-ai site <workspace.json|--stdin> [--strict] [--json|--tasks|--report|--prompts] [--out file] | site --sample [--out file]",
  );
...
test("runHelp delegates command topics to command-specific help", async () => {
  ...
  assert.match(siteOutput, /--out file\s+Write --json, --sample, --tasks, --report, or --prompts output to a file/);
