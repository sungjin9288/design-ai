...
test("runHelp lists advanced options supported by command parsers", async () => {
  ...
  assert.match(output, /site <workspace\.json\|--stdin> \[--strict\] \[--json\|--tasks\|--report\|--prompts\|--prompt id \[--task id\]\] \[--out file\] \| site --sample \[--out file\]/);
  assert.match(
    output,
...
test("runHelp emits a machine-readable help topic catalog", async () => {
  ...
  assert.equal(
    ...
    "design-ai site <workspace.json|--stdin> [--strict] [--json|--tasks|--report|--prompts|--prompt id [--task id]] [--out file] | site --sample [--out file]",
  );
