...
Core design consulting workflows are locally release-ready. Local learning preferences are available through `design-ai learn`, read-only `learn --audit` cleanup suggestions / `learn --stats`, and opt-in `prompt`/`pack --with-learning` with category/limit scoping plus learned-context audit summaries; AI model training or fine-tuning remains outside the shipped scope.
