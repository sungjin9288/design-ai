...
CLI commands include `design-ai start <brief|--from-file file|--stdin> [--route id] [--site-name name] [--repo-url url|--local-path path] [--url url] [--screenshot ref] [--locale locale] [--viewport name] [--json]` alongside the existing install, route, prompt, artifact, pack, learn, check, workspace, site, corpus, audit, MCP, version, and help workflows. Run `design-ai help --json` for the complete machine-readable catalog or `design-ai help <command>` for exact options.
...
The current unreleased source adds the read-only `design-ai start`, SDK `start()`, and MCP `design_ai_start` entry flow, bringing the source contract to 18 MCP tools and 11 SDK exports. All adapters return the same route, design contract, unexecuted review state, next command, and performed/intended effect boundary. Website Console imports that JSON without rerouting the brief or inspecting declared repositories, pages, or screenshots.
