...
The current unreleased source adds the read-only `design-ai start` entry flow, an evidence-backed HTML inspector, five opt-in Korean product review packs, the optional CLI-only browser evidence runner, and `design-ai benchmark` for repeatable specialization proof. `design-ai review`, SDK `reviewHtml()`, MCP `design_ai_review_html`, and Website Console combine the start plan and static report into one canonical session with exact source identity, artifact linkage, ordered stages, and a pending human decision. `design-ai review-handoff`, SDK `reviewHandoff()`, MCP `design_ai_review_handoff`, and Website Console turn that session into a self-validating but undelivered handoff. `design-ai review-handoff-verify`, SDK `verifyReviewHandoff()`, MCP `design_ai_verify_review_handoff`, and Website Console let the named consumer revalidate the exact handoff bytes and issue a separate receipt without claiming identity, transport, or acceptance. CLI `design-ai review-intake`, MCP `design_ai_review_intake`, and Website Console verify the receipt-declared checkout without reading application source. P10 then binds that intake to an immutable request through CLI, SDK `proposeImplementationScope()` / `approveImplementationScope()`, MCP `design_ai_review_scope` / `design_ai_approve_review_scope`, and Website Console. P11 adds CLI `design-ai review-evidence`, MCP `design_ai_review_evidence`, and Website Console import over one read-only Git and evidence-file operation. P12 adds CLI `design-ai review-pilot`, SDK `recordPilotEvidence()`, MCP `design_ai_review_pilot`, and Website Console exact-source import for one consented real pilot. It derives first-value time, finding decisions, approval friction, implementation completion, and unresolved risk while separating real, synthetic, inferred, and unverified claims. Large MCP chains can request `compact: true`, which keeps exact source hashes, byte counts, metrics, claims, issues, and boundaries without returning duplicated nested source bodies. None of these operations establishes external adoption or production quality. Commit, push, deployment, migration execution, and external writes remain separate. `design-ai inspect`, SDK `inspectHtml()`, and MCP `design_ai_inspect_html` share the lower-level eight-lens quality report; product packs expose the same five Korean domain contracts. This brings the source contract to 28 MCP tools and 19 SDK exports. See [Real pilot evidence](docs/REAL-PILOT-EVIDENCE.md).
