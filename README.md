...
CLI commands include `design-ai start <brief|--from-file file|--stdin> ...`, `design-ai inspect <source.html> --brief text ...`, and approval-gated `design-ai verify-browser <quality-report.json> ...` alongside the existing install, route, prompt, artifact, pack, learn, check, workspace, site, corpus, audit, MCP, version, and help workflows. Run `design-ai help --json` for the complete machine-readable catalog or `design-ai help <command>` for exact options.
...
The current unreleased source adds the read-only `design-ai start` entry flow, the first evidence-backed HTML inspector, and the optional CLI-only browser evidence runner. `design-ai inspect`, SDK `inspectHtml()`, and MCP `design_ai_inspect_html` share the canonical eight-lens quality report, bringing the source contract to 19 MCP tools and 12 SDK exports. Static markup defects are confirmed with source locations; interaction, motion, performance, keyboard, accessibility-tree, and rendered responsive behavior stay `unverified` until an approved `design-ai verify-browser` run supplies evidence. The browser runner adds no production browser dependency, accepts loopback previews only, compares the source-report digest after adapter exit, and confines its own sidecar files to `~/.design-ai/evidence/browser/`. Its user-supplied adapter executes only after `--yes`, attests to the requested network policy, and remains an unsandboxed macOS/Linux boundary whose target-repository, external-write, and restored intermediate source-mutation behavior is recorded as `unverified`.
