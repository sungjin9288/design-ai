...
CLI commands include `design-ai start <brief|--from-file file|--stdin> ...` and `design-ai inspect <source.html> --brief text ...` alongside the existing install, route, prompt, artifact, pack, learn, check, workspace, site, corpus, audit, MCP, version, and help workflows. Run `design-ai help --json` for the complete machine-readable catalog or `design-ai help <command>` for exact options.
...
The current unreleased source adds the read-only `design-ai start` entry flow and the first evidence-backed HTML inspector. `design-ai inspect`, SDK `inspectHtml()`, and MCP `design_ai_inspect_html` share the canonical eight-lens quality report, bringing the source contract to 19 MCP tools and 12 SDK exports. Static markup defects are confirmed with source locations; interaction, motion, performance, keyboard, accessibility-tree, and rendered responsive behavior stay `unverified`. No adapter executes scripts, opens a browser, writes a target file, or mutates a repository.
