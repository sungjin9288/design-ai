...
CLI commands include `design-ai start <brief|--from-file file|--stdin> ...`, the canonical `design-ai review <source.html> --brief text ...` workflow, self-validating `design-ai review-handoff <review-workflow.json> --recipient name ...`, `design-ai review-pack [id]`, the lower-level `design-ai inspect <source.html> --brief text --review-pack <id> ...`, and approval-gated `design-ai verify-browser <quality-report.json> ...` alongside the existing install, route, prompt, artifact, pack, learn, check, workspace, site, corpus, audit, MCP, version, and help workflows. Run `design-ai help --json` for the complete machine-readable catalog or `design-ai help <command>` for exact options.
...
| **Node.js / Agent SDK** | `import { artifact, start, reviewHtml, reviewHandoff, inspectHtml, route, prompt, pack, search, recall, check, routes, version } from "@design-ai/cli/sdk"` — call design-ai's deterministic verbs as functions, no CLI shell-out or MCP server needed. Read-only except the explicit `learn.*` namespace. [SDK reference](docs/SDK.md). [Walkthrough](docs/integrations/agent-sdk-walkthrough.md). |
