...
Core design consulting workflows are locally release-ready. Local learning preferences are available through `design-ai learn`, read-only `learn --audit`, and opt-in `prompt`/`pack --with-learning` with category/limit scoping; AI model training or fine-tuning remains outside the shipped scope.
