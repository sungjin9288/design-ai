...
Core design consulting workflows are locally release-ready. Local learning preferences are available through `design-ai learn`, explicit `learn --feedback` keep/improve/avoid guidance, portable `learn --import` dry-run/confirmed profile merge, read-only `learn --audit` cleanup suggestions / `learn --stats`, safe `learn --audit --fix --dry-run` previews plus confirmed `--fix --yes` cleanup, and opt-in `prompt`/`pack --with-learning` with category/limit scoping plus learned-context audit summaries; AI model training or fine-tuning remains outside the shipped scope.
