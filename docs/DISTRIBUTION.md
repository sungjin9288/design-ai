...
- Packed-tarball smoke also verifies `design-ai learn --signals` learning signal registry output in human, JSON, and `--out` modes for installed-bin and one-shot `npm exec --package <tarball>` paths.
