...
design-ai workspace   Show a read-only local dogfood readiness snapshot for git, repository metadata, learning, optional learning eval checkpoints, and release scripts; add --root path, --learning-file path, --learning-eval path, --strict, or --json
...
- Public registry workspace learning-eval coverage includes public registry `design-ai workspace --learning-eval learning-eval.json --strict --json` checkpoint summaries from the published package path.
