...
> Distribution status, checked 2026-07-16: GitHub Release `v5.1.0` and provenance-backed `@design-ai/cli@5.1.0` are public, npm `latest` points to `5.1.0`, and public registry smoke passes. The Homebrew formula is pinned to `v5.1.0` and passed temporary-tap source install plus formula test. GitHub Pages docs are live, and `sungjin.design-ai-vscode@0.4.1` remains public on the VS Code Marketplace. See [`external-status.md`](external-status.md).
...
Use this path for the public package. `@design-ai/cli@5.1.0` is published with provenance and public registry smoke has passed.
