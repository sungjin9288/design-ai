...
- Packed-tarball smoke also verifies `design-ai learn --signals` learning signal registry output, `design-ai learn --signals --strict --json` strict gate output, and `design-ai learn --propose-skills` skill proposal output in human, JSON, and `--out` modes for installed-bin and one-shot `npm exec --package <tarball>` paths.
