...
| Operator Runbook | `design-ai site <bundle-dir> --bundle-handoff --json` 결과를 import해 stage metric, resettable action/evidence row filter, status index, evidence progress, copy-ready stage row, row-level Markdown copy/export와 line copy, filtered/full Markdown export, next-line copy action을 검토 |
