...
| Operator Runbook | `design-ai site <bundle-dir> --bundle-handoff --json` 결과를 import해 source-bundle provenance/detail review, focused source-bundle JSON import, copy-ready strict source command, source-bundle Markdown/JSON copy/export, task/command provenance chip, stage metric, resettable action/evidence row filter, status index, evidence progress, copy-ready stage row, row-level Markdown copy/export와 line copy, filtered/full Markdown export, next-line copy action을 검토 |
