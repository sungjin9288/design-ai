...
design-ai review page.html --brief "Review Korean fintech settings" --repo-url https://github.com/acme/settings --local-path /absolute/path/to/settings --review-pack korean-fintech --locale ko-KR --viewport mobile --viewport desktop --json
...
`design-ai review` is the normal entry point for an existing HTML artifact. It
returns the route and design contract together with the static quality report,
proves their context and SHA-256 linkage, and stops before browser verification or
implementation. Use `inspect` when you need only the lower-level quality report.
See [Canonical review workflow](REVIEW-WORKFLOW.md).

Use `design-ai review-handoff` when another agent or reviewer will consume the
review. It preserves the exact workflow JSON, names the recipient, and stops with
delivery and consumer validation still pending. Add both `--quality-report` and
`--browser-verification` only when the exact browser evidence pair exists. See
[Review evidence handoff](REVIEW-HANDOFF.md).

The receiving agent uses `design-ai review-handoff-verify` to revalidate the exact
handoff bytes and emit a consumer receipt. This proves contract validation only;
identity, transport, acceptance, target-repository intake, and implementation
remain unverified. See [Review handoff validation receipt](REVIEW-HANDOFF-RECEIPT.md).

Next, `design-ai review-intake` checks that the receipt's declared path and remote
match the local checkout, then records root project metadata and Git state. It does
not read application source or authorize implementation. See [Target repository
intake](REVIEW-INTAKE.md).

After implementing only the approved selectors, use `design-ai review-evidence`
before commit. It compares the unchanged approved baseline with current Git state,
records missing verification as missing, and hashes only declared evidence files.
See [Implementation evidence](IMPLEMENTATION-REVIEW-EVIDENCE.md).
