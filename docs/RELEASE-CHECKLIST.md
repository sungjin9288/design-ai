...
Packed-tarball smoke guidance must also retain `design-ai learn --signals` learning signal registry coverage for human, JSON, Markdown signal report via `design-ai learn --signals --report --out learning-signals.md`, learn signals JSON `--out` file-write confirmation, focused `design-ai learn --agent-backlog --report --out agent-backlog.md` Markdown report coverage plus agent backlog JSON `--out` file-write confirmation, and `design-ai learn --signals --strict --json` plus `design-ai learn --agent-backlog --strict --json` strict gates in installed-bin and one-shot `npm exec --package <tarball>` paths.
