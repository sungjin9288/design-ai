...
For a release or publish workflow that builds its own final tarball, run the
non-publishing portion first:

```bash
npm run release:preflight
```

`release:preflight` runs the unit tests, strict audits, whitespace and package
contents checks, release metadata and assertion self-tests, and documentation policy
check. `release:check` adds one packed-tarball smoke. Tag workflows use the preflight,
then smoke-test the exact tarball they are about to release or publish.

Packed-tarball smoke runs `design-ai benchmark --strict --json` through installed-bin and one-shot package paths. This read-only product specialization benchmark covers four synthetic cases, uses no aggregate quality score, and does not prove customer adoption.
