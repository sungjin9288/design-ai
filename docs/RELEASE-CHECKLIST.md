...
Packed-tarball smoke guidance must also retain `design-ai learn --signals` learning signal registry coverage for human, JSON, Markdown signal report via `design-ai learn --signals --report --out learning-signals.md`, `design-ai learn --signals --strict --json` strict gate, and `--out` output in installed-bin and one-shot `npm exec --package <tarball>` paths.
