...
Packed-tarball smoke guidance must also retain `design-ai learn --signals` learning signal registry coverage for human, JSON, `--strict --json`, and `--out` output in installed-bin and one-shot `npm exec --package <tarball>` paths.
