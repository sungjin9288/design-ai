...
Packed-tarball smoke guidance must also retain `design-ai learn --propose-skills` preview-only skill proposal coverage for human, JSON, Markdown review artifact via `design-ai learn --propose-skills --report --out skill-proposals.md`, unified diff handoff via `design-ai learn --propose-skills --patch --out skill-proposals.patch`, JSON `--out` output, and `design-ai learn --propose-skills --min-evidence 3 --json` threshold skipping in installed-bin and one-shot `npm exec --package <tarball>` paths.
