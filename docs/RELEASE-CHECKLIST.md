...
Packed-tarball smoke guidance must also retain `design-ai learn --propose-skills` preview-only skill proposal coverage for human, JSON, Markdown `--report --out`, and JSON `--out` output in installed-bin and one-shot `npm exec --package <tarball>` paths.
