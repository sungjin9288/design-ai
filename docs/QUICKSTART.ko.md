...
`design-ai benchmark`는 패키지에 포함된 네 전문화 사례를 실행하고 종합 품질 점수
없이 계약 유효성, 정확한 finding 비교, 권한 경계, 남은 미검증 위험을 보여줍니다.
[제품 전문화 벤치마크](BENCHMARKS.ko.md)를 참고하세요.

기존 HTML 산출물은 `design-ai review`에서 시작하는 것이 기본입니다. route와
...
```bash
# 유지보수자 전용: refs/ (gitignored 업스트림 소스) 재추출
