...
design-ai learn      local learning preference 관리; --init으로 preview-first starter profile bootstrap을 하고, list/export에서 --query로 matching profile을 확인하고, --backup --json은 전체 portable profile backup, --redact --json / --redact --from-file / --redact --stdin은 공유용 redacted backup을 만들며, --verify / --diff로 portable profile 이동 전 상태를 확인하고, --restore는 preview-first 전체 profile 교체를 수행하고, --curate는 duplicate/sensitive entry를 archive-first 방식으로 정리하며, --out file과 --force로 안전한 artifact 저장을 지원하고, --import는 확인된 profile merge를 수행
