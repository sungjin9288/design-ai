...
- Packed-tarball smoke는 installed-bin과 one-shot `npm exec --package <tarball>` 경로에서 `design-ai learn --signals` learning signal registry의 human, JSON, `--out` output도 확인해요.
...
- Public registry Website Console coverage는 공개 npm registry `design-ai site` Website Console export validation, sample workspace, prompt template 목록, MCP readiness, MCP action plan, handoff bundle, bundle-check/compare/handoff, refactor task 생성, task-selected prompt 생성을 published package path에서 확인해요.
