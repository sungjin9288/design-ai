...
- Packed-tarball smoke는 installed-bin과 one-shot `npm exec --package <tarball>` 경로에서 `design-ai learn --signals` learning signal registry, `design-ai learn --signals --strict --json` strict gate, `design-ai learn --propose-skills` skill proposal의 human, JSON, `--out` output도 확인해요.
