...
> 배포 상태, 2026-07-16 확인: GitHub Release `v5.1.0`과 provenance가 포함된 npm `@design-ai/cli@5.1.0`이 공개됐고 npm `latest`도 `5.1.0`을 가리켜요. public registry smoke가 통과했고, Homebrew formula는 `v5.1.0`에 pinning되어 temporary tap source install과 formula test를 통과했어요. GitHub Pages 문서와 VS Code Marketplace의 `sungjin.design-ai-vscode@0.4.1`도 공개 상태예요. 자세한 내용은 [`external-status.md`](external-status.md)를 확인하세요.
...
public package 설치 경로예요. `@design-ai/cli@5.1.0` publish와 provenance, public registry smoke 통과가 확인됐어요.
...
design-ai inspect file 명시한 HTML 파일 하나에서 canonical 품질 보고서 생성; 정적 finding은 source location을 남기고 runtime 동작은 unverified로 유지
...
`design-ai start ... --json`과 `design-ai inspect ... --json`을 실행해요. 공개 후
...
- npm Trusted Publishing과 GitHub Actions OIDC를 사용해요. npm package 설정에서 repository `sungjin9288/design-ai`와 workflow `publish.yml`을 신뢰하도록 등록하며, 장기 npm token은 job에 주입하지 않아요.
- npm OIDC exchange를 지원하는 Node.js 24와 고정 npm `11.18.0`을 사용해요. `id-token: write`는 publish job에만 부여하고, smoke를 통과한 `dist/*.tgz` artifact만 publish해요.
