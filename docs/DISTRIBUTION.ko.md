...
- 패킹된 tarball을 임시 프로젝트에 설치하고 `design-ai help --json` topic catalog를 읽어 expected public topic/alias set을 검증한 뒤 모든 `design-ai help <command>` topic, 문서화된 help alias, fake `CLAUDE_HOME` 기반 `design-ai install` smoke test 실행.
- `--provenance`로 publish (npm provenance attestation).
- publish 후 공개 npm registry package를 `npm exec --package @design-ai/cli@<version>` 경로로 smoke test하고, expected `design-ai help --json` catalog, 발견된 help topic, 문서화된 help alias도 함께 검증.
