...
```
design-ai install     ~/.claude로 design-ai 심볼릭 링크; --json으로 machine-readable install count 출력
...
design-ai version     CLI + 플러그인 버전; --json으로 machine-readable metadata 출력
...
- 패킹된 tarball을 임시 프로젝트에 설치해 packed-tarball installed-bin 경로를 smoke test하고 같은 public CLI surface를 one-shot `npm exec --package <tarball>` 경로로 다시 검증하며, human/JSON `design-ai version`과 `design-ai help` top-level help 출력을 검증한 뒤 `design-ai help --json` topic catalog를 읽어 expected public topic/alias set을 확인하고, 모든 `design-ai help <command>` topic-specific usage 출력, 문서화된 help/command alias 출력, `find`, `cat`, `recommend`, `example`, `ex`, `ls`, `lint` functional alias 출력, 세 가지 `list` catalog domain의 human/JSON 출력, human/JSON `search` / `show` / `examples` 출력, route JSON/catalog/stdin 출력, 명시적 `show --lines` range와 `route --explain` 출력, unknown command/help/list/search-dir failure, unknown route-id/option/value suggestion 및 numeric range failure 검증, prompt/pack JSON/markdown/from-file/stdin 출력, prompt/pack 강제 `--out` overwrite와 `Wrote <path>` confirmation, check examples/artifact/stdin/all-routes 출력, human/JSON `audit --strict --quiet`, human/JSON `design-ai update --dry-run`, fake `CLAUDE_HOME` 기반 human `design-ai install`, JSON `design-ai install --json`, `doctor --strict`, human+JSON `status`, human uninstall과 JSON `uninstall --json` lifecycle 출력까지 검증.
- `--provenance`로 publish (npm provenance attestation).
- publish 후 공개 npm registry package를 `npm exec --package @design-ai/cli@<version>` 경로로 smoke test하고, human/JSON version과 `design-ai help` top-level help 출력, expected `design-ai help --json` catalog, 발견된 help topic usage 출력, 문서화된 help/command alias 출력, `find`, `cat`, `recommend`, `example`, `ex`, `ls`, `lint` functional alias 출력, 세 가지 `list` catalog domain의 human/JSON 출력, human/JSON corpus discovery 출력, route JSON/catalog/stdin 출력, 명시적 `show --lines` range와 `route --explain` 출력, unknown command/help/list/search-dir failure, unknown route-id/option/value suggestion 및 numeric range failure 검증, prompt/pack JSON/markdown/from-file/stdin 출력, prompt/pack 강제 output-file confirmation, check examples/artifact/stdin/all-routes 출력, human/JSON `audit --strict --quiet`, human/JSON `update --dry-run`, human install과 JSON `install --json`, `doctor --strict`, human+JSON status, human uninstall과 JSON `uninstall --json` lifecycle 출력도 함께 검증.
