...
- Imports `design-ai site <bundle-dir> --bundle-handoff --json` output as an Operator Runbook in the Report tab, including stage metrics, action/evidence row filters, status indexes, evidence progress, copy-ready stage rows, filtered-row Markdown copy/export, full Markdown export, and next-line copy actions.
