...
- Imports `design-ai site <bundle-dir> --bundle-handoff --json` output as an Operator Runbook in the Report tab, including source-bundle provenance/detail review, focused source-bundle JSON import, copy-ready strict source commands, source-bundle Markdown/JSON copy/export, task/command provenance chips, stage metrics, resettable action/evidence row filters, status indexes, evidence progress, copy-ready stage rows, row-level Markdown copy/export and line copy, filtered-row Markdown copy/export, full Markdown export, and next-line copy actions.
