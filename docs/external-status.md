...
Local release readiness is verified, GitHub Pages docs are publicly reachable, `@design-ai/cli@4.55.0` is published on npm with public registry smoke coverage, and the Homebrew tap formula points at the `v4.55.0` release source tarball with a verified SHA-256. The VS Code extension `sungjin.design-ai-vscode` is published to the VS Code Marketplace at version `0.4.1`; the Gallery API is reachable. The public npm MCP entrypoint and the local Claude Code / Codex MCP client registrations were rechecked on 2026-07-02.
