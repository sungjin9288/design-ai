...
2. Run `design-ai version`, top-level `help`, `help --json`, every expected command-specific help topic from the catalog with usage assertions, every documented help alias, every documented command alias with output assertions, all three `list` catalog domains, human / JSON `search`, `show`, and `examples`, and prompt/pack forced file-output flows.
