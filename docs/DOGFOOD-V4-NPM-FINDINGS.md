...
- runs `version`, top-level `help`, `help --json`, every expected public `design-ai help <command>` topic, every documented help alias, `list skills`, `install`, `doctor --json`, `doctor --strict`, `status`, and `uninstall`
...
- runs a local tarball `npm exec --package ... -- design-ai ...` path to simulate one-shot `npx`, including its own `help --json` catalog read and help alias smoke
...
2. Run `design-ai version`, top-level `help`, `help --json`, every expected command-specific help topic from the catalog, every documented help alias, and `list skills`.
...
6. Run the same lifecycle through local tarball `npm exec --package ...` to cover the `npx`-style bin path, including an independent `help --json` catalog read, documented help alias smoke, and the same `doctor --json` PASS assertions.
