...
| Local AI learning preferences | Complete for v4.13 | `design-ai learn`, `prompt --with-learning`, `pack --with-learning`, and confirmed `learn --forget`/`--clear` controls provide explicit local preference memory | Keep privacy boundaries clear as learning scope expands |
...
- Store, filter, export, forget, and clear explicit local learning preferences, and inject them into prompts/packs only when requested.
