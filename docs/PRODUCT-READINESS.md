...
As of v4.13, these are covered by the manifest, commands, skills, agents, examples, and release smoke suite, including packed-tarball checks for workspace strict failure/success readiness behavior, public registry checks for the same workspace strict contract after publish, check learning capture, learning feedback, backup, redaction, learn JSON `--out` file-write confirmation and forced overwrite coverage, verify, import, query-filtered learn list explanation/export, brief-relevant prompt/pack learning selection, and learning audit cleanup suggestions. Public registry smoke also verifies learning feedback/init bootstrap, portable learning import/redact behavior, query-filtered learn list/export behavior, brief-relevant prompt/pack learning selection, and learning audit cleanup suggestions plus safe cleanup dry-run/apply behavior after publish.
