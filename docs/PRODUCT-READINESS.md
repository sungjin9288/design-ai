...
| Local AI learning preferences | Complete for v4.13 | `design-ai learn`, filtered `prompt --with-learning` / `pack --with-learning`, confirmed `learn --forget`/`--clear`, and non-mutating `learn --audit` controls provide explicit local preference memory | Keep privacy boundaries clear as learning scope expands |
...
- Store, filter, audit, export, forget, and clear explicit local learning preferences, and inject scoped category/limit subsets into prompts/packs only when requested.
