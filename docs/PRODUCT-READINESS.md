...
| Internal dogfood readiness | Complete for v4.13 | `design-ai workspace` reports read-only git cleanliness/sync, canonical repository remote/metadata alignment, local learning profile audit state, release-script availability, and next-action hints before solo/internal distribution work; `--strict` exits non-zero on readiness warnings/failures | Use findings to decide whether the next surface should be CLI, web UI, VS Code, Figma, or SDK |
