...
| Local AI learning preferences | Complete for v4.13 | `design-ai learn`, explicit `learn --feedback` keep/improve/avoid guidance, full `learn --backup --json` export, redacted `learn --redact --json` sharing export, non-mutating `learn --verify`, confirmed `learn --import`, filtered `prompt --with-learning` / `pack --with-learning`, confirmed `learn --forget`/`--clear`, non-mutating `learn --audit` cleanup suggestions / `learn --stats`, safe `learn --audit --fix --dry-run` previews plus confirmed `--fix --yes` cleanup, and learned-context audit summaries provide explicit local preference memory | Keep privacy boundaries clear as learning scope expands |
