...
| Local AI learning preferences | Complete for v4.13 | `design-ai learn`, `prompt --with-learning`, `pack --with-learning`, confirmed `learn --forget`/`--clear`, and non-mutating `learn --audit` controls provide explicit local preference memory | Keep privacy boundaries clear as learning scope expands |
...
- Store, filter, audit, export, forget, and clear explicit local learning preferences, and inject them into prompts/packs only when requested.
...
These are valid future product ideas, but they are different from the current architecture. The current architecture is deterministic corpus routing, prompt packing, quality checking, local preference injection, and release-smoked CLI distribution.
