...
from smoke_domains.package_failures import fail_package_smoke, require_package_smoke
from smoke_domains.package_lifecycle import (
    assert_install_json_smoke,
    assert_install_smoke,
    assert_pilot_evidence_smoke,
    assert_ranked_search_determinism_smoke,
    assert_route_eval_json,
...
from smoke_domains.package_selftest_core import (
    _self_test_check_site_and_mcp,
    _self_test_learn_agent_backlog,
    _self_test_learn_audit_cleanup,
)
from smoke_domains.package_selftest_learning_profile import (
    _self_test_learn_init_import_backup,
    _self_test_learn_query_and_export,
    _self_test_learn_restore_and_prune,
)
from smoke_domains.package_selftest_learning_signals import (
    _self_test_learn_proposal_reports,
    _self_test_learn_skill_proposals,
    _self_test_learn_usage_and_signals,
)
from smoke_domains.package_selftest_learning_review import (
    _self_test_learn_apply_plan,
    _self_test_learn_proposal_review,
    _self_test_learn_proposal_review_files,
)
from smoke_domains.package_selftest_learning_apply import (
    _self_test_learn_apply_plan_artifacts,
    _self_test_learn_apply_plan_evidence,
    _self_test_learn_apply_plan_identity,
)
from smoke_domains.package_selftest_learning_eval import (
    _self_test_learn_audit_and_curation,
    _self_test_learn_eval_template,
)
from smoke_domains.cli_assertions import (
    ...
    read_forced_json_output_file,
    read_forced_markdown_output_file,
...
const handoffSource = JSON.stringify(handoff1, null, 2);
const receipt1 = verifyReviewHandoff(handoffSource, {
  handoffRef: "sdk-review-handoff.json",
  consumer: "package-smoke-agent",
});
const receipt2 = verifyReviewHandoff(handoffSource, {
  handoffRef: "sdk-review-handoff.json",
  consumer: "package-smoke-agent",
});
if (JSON.stringify(receipt1) !== JSON.stringify(receipt2)) {
  throw new Error("verifyReviewHandoff() not deterministic");
}
if (receipt1.kind !== "design-ai-review-handoff-receipt"
  || receipt1.status !== "contract-validated"
  || receipt1.consumer.identity !== "self-declared"
  || receipt1.consumer.acceptance !== "not-claimed"
  || receipt1.nextAction.implementationAuthorized
    ...
    with tempfile.TemporaryDirectory(prefix="design-ai-package-smoke-") as tmp:
        ...
        installed_review_receipt_source = assert_review_handoff_receipt_smoke(
            [
                ...
                str(installed_review_handoff),
                "--consumer",
                "package-smoke-agent",
                "--json",
            ],
            installed_review_handoff,
            consumer="package-smoke-agent",
            env=smoke_env,
