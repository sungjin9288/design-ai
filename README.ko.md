...
CLI에는 `design-ai start <brief|--from-file file|--stdin> ...`, `design-ai inspect <source.html> --brief text ...`, 승인 후 실행하는 `design-ai verify-browser <quality-report.json> ...`와 기존 install, route, prompt, artifact, pack, learn, check, workspace, site, corpus, audit, MCP, version, help workflow가 있어요. 전체 machine-readable 목록은 `design-ai help --json`, 각 명령의 정확한 옵션은 `design-ai help <command>`로 확인하세요.
