...
- 릴리스 PR 또는 태그 전에는 `npm run release:check`를 실행해요. CLI unit test, 8개 audit, whitespace check, package contents check, `npm run release:self-test`, human `design-ai version`과 JSON `design-ai version --json`, top-level help, command alias help와 functional alias 출력, command-specific help topic 출력 검증, 명시적 `show --lines`와 `route --explain` 출력 검증, unknown route-id/option/value suggestion 및 numeric range failure 검증, prompt/pack 강제 `--out` overwrite 및 file-write confirmation, human/JSON `audit --strict --quiet`, `doctor --strict` human diagnostics, human install과 JSON `install --json`, human+JSON status, human uninstall, JSON `uninstall --json` lifecycle 출력 검증을 포함한 packed-tarball smoke를 한 번에 확인해요.
- npm publish가 끝난 뒤에는 `npm run registry:smoke`로 공개 `npm exec --package` 설치 경로, human `design-ai version`과 JSON `design-ai version --json`, help topic catalog, functional alias 출력, 세 가지 `list` catalog domain, human / JSON corpus discovery 출력, 명시적 `show --lines`와 `route --explain` 출력 검증, unknown route-id/option/value suggestion 및 numeric range failure 검증, prompt/pack 강제 `--out` overwrite 및 file-write confirmation, human/JSON `audit --strict --quiet`, `doctor --strict` human diagnostics, human install과 JSON `install --json`, human+JSON status, human uninstall, JSON `uninstall --json` lifecycle 출력을 확인해요.
