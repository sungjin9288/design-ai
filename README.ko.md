...
- 릴리스 PR 또는 태그 전에는 `npm run release:check`를 실행해요. CLI unit test, 7개 audit, whitespace check, package contents check, `npm run release:self-test`, version/top-level help/command alias help/command-specific help topic 출력 검증, unknown route-id/option/value suggestion 및 numeric range failure 검증, prompt/pack 강제 `--out` overwrite 및 file-write confirmation, `doctor --strict` human diagnostics, install/status/uninstall lifecycle 출력 검증을 포함한 packed-tarball smoke를 한 번에 확인해요.
- npm publish가 끝난 뒤에는 `npm run registry:smoke`로 공개 `npm exec --package` 설치 경로, help topic catalog, 세 가지 `list` catalog domain, human / JSON corpus discovery 출력, unknown route-id/option/value suggestion 및 numeric range failure 검증, prompt/pack 강제 `--out` overwrite 및 file-write confirmation, `doctor --strict` human diagnostics, install/status/uninstall lifecycle 출력을 확인해요.
