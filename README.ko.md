...
핵심 디자인 컨설팅 워크플로우는 로컬 릴리스 기준으로 준비되어 있어요. 로컬 학습 선호도는 `design-ai learn`, 읽기 전용 `learn --audit` / `learn --stats`, category/limit 범위 지정과 audit summary가 붙는 선택형 `prompt`/`pack --with-learning`으로 사용할 수 있고, AI 모델 학습이나 fine-tuning은 여전히 현재 배포 범위 밖이에요.
