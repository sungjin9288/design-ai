...
CLI에는 `design-ai start <brief|--from-file file|--stdin> ...`, 표준 `design-ai review <source.html> --brief text ...` workflow, 스스로 검증하는 `design-ai review-handoff <review-workflow.json> --recipient name ...`, 수신자가 사용하는 `design-ai review-handoff-verify <review-handoff.json> --consumer name ...`, 제한된 범위의 `design-ai review-intake <receipt.json> --target-root path --consumer name ...`, `design-ai review-pack [id]`, 하위 수준의 `design-ai inspect <source.html> --brief text --review-pack <id> ...`, 승인 후 실행하는 `design-ai verify-browser <quality-report.json> ...`와 기존 install, route, prompt, artifact, pack, learn, check, workspace, site, corpus, audit, MCP, version, help workflow가 있어요. 전체 machine-readable 목록은 `design-ai help --json`, 각 명령의 정확한 옵션은 `design-ai help <command>`로 확인하세요.
...
| **Node.js / Agent SDK** | `import { artifact, start, reviewHtml, reviewHandoff, verifyReviewHandoff, inspectHtml, route, prompt, pack, search, recall, check, routes, version } from "@design-ai/cli/sdk"` — CLI 셸 실행이나 MCP 서버 없이 design-ai의 결정적 기능을 함수로 바로 호출해요. 명시적인 `learn.*` namespace를 제외하면 읽기 전용이에요. [SDK 레퍼런스](docs/SDK.md). [워크스루](docs/integrations/agent-sdk-walkthrough.md). |
...
현재 미배포 소스에는 읽기 전용 `design-ai start` 진입 흐름, 근거 기반 HTML 품질 검사기, 명시적으로 선택하는 한국형 제품 리뷰 팩 5개, 선택형 CLI browser evidence runner, 반복 가능한 전문화 증빙용 `design-ai benchmark`가 포함되어 있어요. `design-ai review`, SDK `reviewHtml()`, MCP `design_ai_review_html`, Website Console은 시작 계획과 정적 보고서를 정확한 source identity, artifact linkage, 순서가 있는 단계, 사람의 다음 결정이 포함된 하나의 표준 세션으로 결합해요. `design-ai review-handoff`, SDK `reviewHandoff()`, MCP `design_ai_review_handoff`, Website Console은 그 세션을 스스로 검증하지만 아직 전달되지 않은 인계로 준비해요. `design-ai review-handoff-verify`, SDK `verifyReviewHandoff()`, MCP `design_ai_verify_review_handoff`, Website Console은 지정 수신자가 정확한 handoff 원본을 다시 검증하고 별도 영수증을 만들되 신원, 전송, 수락은 주장하지 않아요. CLI `design-ai review-intake`, MCP `design_ai_review_intake`, Website Console은 영수증에 선언된 local checkout의 제한된 루트 metadata와 Git 상태를 확인하되 application source를 읽거나 구현을 허가하지 않아요. `design-ai inspect`, SDK `inspectHtml()`, MCP `design_ai_inspect_html`은 하위 수준의 8개 lens 품질 보고서를 공유하고, `design-ai review-pack`, SDK `reviewPack()`, MCP `design_ai_review_pack`은 핀테크·커머스·SaaS·콘텐츠·게임 계약을 같은 형태로 제공해요. source contract는 24 MCP tools와 16 SDK exports예요. CLI 전용 benchmark는 새 디자인, 기존 제품 리팩터링, 한국형 제품 UX, 멀티 에이전트 인계를 다루는 합성 사례 4개를 패키지에서 실행해요. 종합 품질 점수나 실제 고객 채택 주장 없이 계약 유효성과 정확한 finding ID를 비교합니다. 정적 markup에서 확인한 결함은 위치와 함께 `confirmed`로 기록하고, interaction·motion·performance·keyboard·접근성 트리·한국어 줄바꿈과 밀도·결제 또는 확률 고지·실제 responsive 동작은 승인된 browser 또는 scenario 근거가 생기기 전까지 `unverified`로 남겨요. Website Console은 가져온 intake, receipt, handoff, review, quality, browser JSON 원본 바이트를 보존하고 근거를 승격하지 않은 채 각 계약을 보여 주며 sidecar digest와 viewport 범위를 확인해요. Browser runner는 production browser dependency를 추가하지 않고 loopback preview만 받으며, adapter 종료 뒤 source-report digest를 다시 비교하고 자체 sidecar 파일만 `~/.design-ai/evidence/browser/` 아래에 저장해요. 사용자가 지정한 adapter는 `--yes` 이후 macOS/Linux에서 실행되어 요청된 network policy를 스스로 증명하지만 sandbox 안에서 동작하지 않으므로, target repository 수정, external write, 실행 중 변경했다가 복원한 source 여부는 `unverified`로 기록해요.
