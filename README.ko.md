...
CLI에는 `design-ai start <brief|--from-file file|--stdin> ...`, `design-ai inspect <source.html> --brief text ...`와 기존 install, route, prompt, artifact, pack, learn, check, workspace, site, corpus, audit, MCP, version, help workflow가 있어요. 전체 machine-readable 목록은 `design-ai help --json`, 각 명령의 정확한 옵션은 `design-ai help <command>`로 확인하세요.
...
현재 미배포 소스에는 읽기 전용 `design-ai start` 진입 흐름과 첫 번째 HTML 품질 검사기가 포함되어 있어요. `design-ai inspect`, SDK `inspectHtml()`, MCP `design_ai_inspect_html`이 같은 8개 lens 품질 보고서를 사용하며 source contract는 19 MCP tools와 12 SDK exports예요. 정적 markup에서 확인한 결함은 위치와 함께 `confirmed`로 기록하고, interaction·motion·performance·keyboard·접근성 트리·실제 responsive 동작은 `unverified`로 남겨요. 어떤 adapter도 script를 실행하거나 browser를 열거나 대상 파일과 repository를 변경하지 않아요.
