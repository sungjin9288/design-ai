...
CLI에는 `design-ai start <brief|--from-file file|--stdin> [--route id] [--site-name name] [--repo-url url|--local-path path] [--url url] [--screenshot ref] [--locale locale] [--viewport name] [--json]`과 기존 install, route, prompt, artifact, pack, learn, check, workspace, site, corpus, audit, MCP, version, help workflow가 있어요. 전체 machine-readable 목록은 `design-ai help --json`, 각 명령의 정확한 옵션은 `design-ai help <command>`로 확인하세요.
...
현재 미배포 소스에는 읽기 전용 `design-ai start`, SDK `start()`, MCP `design_ai_start` 진입 흐름이 추가되어 source contract가 18 MCP tools와 11 SDK exports가 되었어요. 모든 adapter가 같은 route, design contract, 실행하지 않은 review 상태, 다음 command, performed/intended effect boundary를 반환해요. Website Console은 brief를 다시 routing하거나 선언된 repository, page, screenshot을 읽지 않고 이 JSON을 가져와 검토하고 다시 내보내요.
