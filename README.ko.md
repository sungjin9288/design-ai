...
| **Node.js / Agent SDK** | `import { artifact, start, reviewHtml, reviewHandoff, verifyReviewHandoff, recordPilotEvidence, inspectHtml, route, prompt, pack, search, recall, check, routes, version } from "@design-ai/cli/sdk"` — CLI 셸 실행이나 MCP 서버 없이 design-ai의 결정적 기능을 함수로 바로 호출해요. 명시적인 `learn.*` namespace를 제외하면 읽기 전용이에요. [SDK 레퍼런스](docs/SDK.md). [워크스루](docs/integrations/agent-sdk-walkthrough.md). |
...
현재 미배포 소스는 P11 구현 증빙 뒤에 P12 실제 파일럿 증빙을 둡니다. CLI `review-pilot`, SDK `recordPilotEvidence()`, MCP `design_ai_review_pilot`, Website Console이 P11 구현 증빙, 원본 P6 워크플로우, 운영자가 작성한 파일럿 기록을 정확한 바이트와 SHA-256으로 연결합니다. 첫 유효 산출물 시간, finding 판단, 승인 마찰, 구현 상태, 미해결 위험을 계산하면서 실제·합성·추론·미확인 주장을 분리합니다. 큰 MCP 체인은 `compact: true`로 중복 원문 본문을 생략하되 정확한 원본 해시, 바이트 수, 지표, 주장, 이슈, 경계를 유지합니다. 내부 dogfood는 증명할 수 있지만 외부 채택과 운영 품질은 주장하지 않습니다. source contract는 28 MCP tools와 19 SDK exports입니다. 자세한 내용은 [`docs/REAL-PILOT-EVIDENCE.ko.md`](docs/REAL-PILOT-EVIDENCE.ko.md)를 참고하세요.
