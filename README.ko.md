...
현재 미배포 소스는 P10 승인 뒤에 P11 구현 증빙 단계를 둡니다. CLI `review-evidence`, MCP `design_ai_review_evidence`, Website Console이 승인 당시 기준점과 현재 Git 상태, 선언한 증빙 파일을 같은 계약으로 확인합니다. 기록한 명령을 대신 실행하지 않으며 미실행 검증과 미확인 관찰을 그대로 남깁니다. source contract는 27 MCP tools와 18 SDK exports이고, Website Console은 `증빙 -> 승인 -> 제안서 -> intake` 순서로 원본을 복원합니다. 자세한 내용은 [`docs/IMPLEMENTATION-REVIEW-EVIDENCE.ko.md`](docs/IMPLEMENTATION-REVIEW-EVIDENCE.ko.md)를 참고하세요.
