...
현재 미배포 소스는 intake 뒤에 별도 P10 구현 범위 승인 단계를 둡니다. CLI `review-scope` / `review-scope-approve`, SDK `proposeImplementationScope()` / `approveImplementationScope()`, MCP `design_ai_review_scope` / `design_ai_approve_review_scope`, Website Console이 같은 원본과 gate를 검증합니다. 승인은 기록된 selector만 허용하며 commit, push, deployment, migration 실행, 외부 쓰기는 별도로 남습니다. source contract는 26 MCP tools와 18 SDK exports이고, Website Console은 `승인 -> 제안서 -> intake` 순서로 원본을 복원합니다. 자세한 내용은 [`docs/IMPLEMENTATION-SCOPE.ko.md`](docs/IMPLEMENTATION-SCOPE.ko.md)를 참고하세요.
