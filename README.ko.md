...
CLI에는 `design-ai start <brief|--from-file file|--stdin> ...`, `design-ai review-pack [id]`, `design-ai inspect <source.html> --brief text --review-pack <id> ...`, 승인 후 실행하는 `design-ai verify-browser <quality-report.json> ...`와 기존 install, route, prompt, artifact, pack, learn, check, workspace, site, corpus, audit, MCP, version, help workflow가 있어요. 전체 machine-readable 목록은 `design-ai help --json`, 각 명령의 정확한 옵션은 `design-ai help <command>`로 확인하세요.
...
| **Node.js / Agent SDK** | `import { artifact, start, route, prompt, pack, search, recall, check, routes, version } from "@design-ai/cli/sdk"` — CLI 셸 실행이나 MCP 서버 없이 design-ai의 결정적 기능을 함수로 바로 호출해요. 명시적인 `learn.*` namespace를 제외하면 읽기 전용이에요. [SDK 레퍼런스](docs/SDK.md). [워크스루](docs/integrations/agent-sdk-walkthrough.md). |
...
현재 미배포 소스에는 읽기 전용 `design-ai start` 진입 흐름, 근거 기반 HTML 품질 검사기, 명시적으로 선택하는 한국형 제품 리뷰 팩 5개, 선택형 CLI browser evidence runner, 반복 가능한 전문화 증빙용 `design-ai benchmark`가 포함되어 있어요. `design-ai inspect`, SDK `inspectHtml()`, MCP `design_ai_inspect_html`이 같은 8개 lens 품질 보고서를 사용하고, `design-ai review-pack`, SDK `reviewPack()`, MCP `design_ai_review_pack`이 핀테크·커머스·SaaS·콘텐츠·게임 계약을 같은 형태로 제공해요. source contract는 20 MCP tools와 13 SDK exports예요. CLI 전용 benchmark는 새 디자인, 기존 제품 리팩터링, 한국형 제품 UX, 멀티 에이전트 인계를 다루는 합성 사례 4개를 패키지에서 실행해요. 종합 품질 점수나 실제 고객 채택 주장 없이 계약 유효성과 정확한 finding ID를 비교합니다. 정적 markup에서 확인한 결함은 위치와 함께 `confirmed`로 기록하고, interaction·motion·performance·keyboard·접근성 트리·한국어 줄바꿈과 밀도·결제 또는 확률 고지·실제 responsive 동작은 승인된 browser 또는 scenario 근거가 생기기 전까지 `unverified`로 남겨요. Website Console은 가져온 quality/browser JSON 원본 바이트를 보존하고 두 계약을 분리해 보여 주며, 근거를 합치지 않은 채 sidecar digest와 viewport 범위를 확인해요. Browser runner는 production browser dependency를 추가하지 않고 loopback preview만 받으며, adapter 종료 뒤 source-report digest를 다시 비교하고 자체 sidecar 파일만 `~/.design-ai/evidence/browser/` 아래에 저장해요. 사용자가 지정한 adapter는 `--yes` 이후 macOS/Linux에서 실행되어 요청된 network policy를 스스로 증명하지만 sandbox 안에서 동작하지 않으므로, target repository 수정, external write, 실행 중 변경했다가 복원한 source 여부는 `unverified`로 기록해요.
