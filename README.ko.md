...
CLI에는 `design-ai start <brief|--from-file file|--stdin> ...`, 표준 `design-ai review <source.html> --brief text ...` workflow, 스스로 검증하는 `design-ai review-handoff <review-workflow.json> --recipient name ...`, `design-ai review-pack [id]`, 하위 수준의 `design-ai inspect <source.html> --brief text --review-pack <id> ...`, 승인 후 실행하는 `design-ai verify-browser <quality-report.json> ...`와 기존 install, route, prompt, artifact, pack, learn, check, workspace, site, corpus, audit, MCP, version, help workflow가 있어요. 전체 machine-readable 목록은 `design-ai help --json`, 각 명령의 정확한 옵션은 `design-ai help <command>`로 확인하세요.
...
| **Node.js / Agent SDK** | `import { artifact, start, reviewHtml, reviewHandoff, inspectHtml, route, prompt, pack, search, recall, check, routes, version } from "@design-ai/cli/sdk"` — CLI 셸 실행이나 MCP 서버 없이 design-ai의 결정적 기능을 함수로 바로 호출해요. 명시적인 `learn.*` namespace를 제외하면 읽기 전용이에요. [SDK 레퍼런스](docs/SDK.md). [워크스루](docs/integrations/agent-sdk-walkthrough.md). |
