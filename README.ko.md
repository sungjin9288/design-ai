...
현재 소스는 20개 스킬, 16개 공개 슬래시 명령어, 4개 에이전트를 `~/.claude/`에 `design-` 접두사로 설치해요. Claude Code를 다시 실행하고 시도해 보세요:
...
│
├── examples/                # 225개 워크드 예제 ("good"이 어떻게 생겼는지)
│
...
전체 단계 로그는 [`docs/ROADMAP.md`](docs/ROADMAP.md), 현재 완료 범위는 [`docs/PRODUCT-READINESS.md`](docs/PRODUCT-READINESS.md)에서 확인하세요. **v4.65.0**은 CLI–SDK–MCP 패리티와 MCP 14 tools를 제공하는 현재 공개 버전이에요. 현재 소스는 **v5.0.0 release candidate**로, `agentic-design-development` route와 승인 기반 홈페이지 개발/리팩터링을 위한 15번째 read-only `design_ai_site_bundle_handoff` tool을 추가하고, dashboard chart knowledge 보강, token extraction clone-only 분리, public contract와 PR 검증 강화를 포함해요. 이전 public `extract-tokens` command 제거를 major version boundary로 명확히 기록했어요.
